...
void COpenCVWithMFCDlg::SendVideoToAI(const std::string& filepath, const std::string& cameraName)
{
    ...
    HINTERNET hConnect = WinHttpConnect(hSession, L"f1e1-155-230-28-29.ngrok-free.app", INTERNET_DEFAULT_HTTPS_PORT, 0);
...
void COpenCVWithMFCDlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    if (nChar == VK_F5)  // F5 키
    {
        CString log = _T("테스트 영상 전송 시작: VideoForTest.mp4");
        AddLog(log);
