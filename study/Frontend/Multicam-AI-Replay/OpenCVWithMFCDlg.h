...
private:
	...
	std::vector<cv::Mat> m_frameBuffer[4];  // 카메라별 프레임 버퍼
	int m_frameCount = 60;                  // 2초 동안의 프레임 수 (30fps 기준)
	void SaveAndSendVideo(int cameraIndex);
	void SendVideoToAI(const std::string& filepath, const std::string& cameraName);
