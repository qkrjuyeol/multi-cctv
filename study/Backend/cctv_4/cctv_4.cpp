...
struct ONVIFCameraConfig {
    ...
    ONVIFCameraConfig(
        ...
        int _segmentDuration = 60,  // 10분 세그먼트 (15초->600초->15초)
    ...
    void recordThreadFunc() {
        ...
        while (isRunning) {
            try {
                ...
                av_dict_set(&encoderOptions, "profile", "100", 0);           // 프로필 설정 (baseline, main, high)
                av_dict_set(&encoderOptions, "level", "4.1", 0);              // 레벨 설정


                // Media Foundation 특화 설정
                // 키프레임 간격 설정 방법 변경
                av_dict_set(&encoderOptions, "gop_size", std::to_string(currentFps * 2).c_str(), 0);  // 키프레임 간격
                // 낮은 지연 설정 시도
...
int main() {
    ...
    multiRecorder.addCamera(ONVIFCameraConfig(
        "IPCamera1",                           // 카메라 이름
        "192.168.219.150",                    // IP 주소
        554,                                  // RTSP 표준 포트
        "admin",                              // 사용자 이름 (카메라 설정에 맞게 변경)
        "Windo4101!",                           // 비밀번호 (카메라 설정에 맞게 변경)
        "rtsp://admin:Windo4101!@192.168.219.150:554/stream1",  // RTSP URL (카메라의 실제 스트림 경로로 변경)
        "C:/Download/savedfiles"              // 저장 경로
    ));

    //multiRecorder.addCamera(ONVIFCameraConfig(
    //    "IPCamera2",                           // 카메라 이름
    //    "192.168.50.164",                    // IP 주소
    //    554,                                  // RTSP 표준 포트
    //    "admin",                              // 사용자 이름 (카메라 설정에 맞게 변경)
    //    "Windo4101!",                           // 비밀번호 (카메라 설정에 맞게 변경)
    //    "rtsp://admin:Windo4101!@192.168.50.164:554/stream1",  // RTSP URL (카메라의 실제 스트림 경로로 변경)
    //    "C:/Download/savedfiles"              // 저장 경로
    //));

    multiRecorder.addCamera(ONVIFCameraConfig(
        "IPCamera3",                           // 카메라 이름
        "192.168.219.200",                    // IP 주소
        554,                                  // RTSP 표준 포트
        "admin",                              // 사용자 이름 (카메라 설정에 맞게 변경)
        "Windo4101!",                           // 비밀번호 (카메라 설정에 맞게 변경)
        "rtsp://admin:Windo4101!@192.168.219.200:554/stream1",  // RTSP URL (카메라의 실제 스트림 경로로 변경)
        "C:/Download/savedfiles"              // 저장 경로
    ));

    multiRecorder.addCamera(ONVIFCameraConfig(
        "IPCamera4",                           // 카메라 이름
        "192.168.219.135",                    // IP 주소
        554,                                  // RTSP 표준 포트
        "admin",                              // 사용자 이름 (카메라 설정에 맞게 변경)
        "Windo4101!",                           // 비밀번호 (카메라 설정에 맞게 변경)
        "rtsp://admin:Windo4101!@192.168.219.135:554/stream1",  // RTSP URL (카메라의 실제 스트림 경로로 변경)
        "C:/Download/savedfiles"              // 저장 경로
    ));
